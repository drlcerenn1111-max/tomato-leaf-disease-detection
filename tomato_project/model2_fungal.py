"""
Model 2 - Fungal/Oomycete Alt Sınıflandırıcı
---------------------------------------------
Hiyerarşik sınıflandırmanın ikinci aşaması.
Sadece Fungal/Oomycete grubunu 5 alt sınıfa ayırır:
    0: Early Blight      (Alternaria solani)
    1: Late Blight       (Phytophthora infestans)
    2: Leaf Mold         (Passalora fulva)
...
# ── Veriyi yükle ve filtrele (sadece Fungal/Oomycete) ─────────────────────────
print("Veri yükleniyor...")
data = pd.read_csv("/Users/cerendereli/Desktop/tomato-leaf-disease-detection/tomato_project/tomato_all_features.csv")

# Sadece broad_target == 2 (Fungal/Oomycete) olan satırları al
data_fungal = data[data['broad_target'] == 2].copy()

X = data_fungal[FEATURE_COLS]
y = data_fungal['fungal_target']  # Model 2 hedefi

print(f"Fungal alt veri kümesi şekli: {data_fungal.shape}")
...
for _ in tqdm(range(1000), desc="Model 2 eğitiliyor"):
    # Veriyi karıştır
    data_shuffled = data_fungal.sample(frac=1, random_state=None)
    X_shuffled = data_shuffled[FEATURE_COLS]
    y_shuffled = data_shuffled['fungal_target']
...
# ── Sonuçlar ──────────────────────────────────────────────────────────────────
print(f"\n{'='*50}")
print("MODEL 2 — FUNGAL ALT SINIFLANDIRICI SONUÇLARI")
print(f"{'='*50}")
...
data_shuffled = data_fungal.sample(frac=1, random_state=0)
X_train, X_test, y_train, y_test = train_test_split(
    data_shuffled[FEATURE_COLS], data_shuffled['fungal_target'],
    test_size=0.20, random_state=42, stratify=data_shuffled['fungal_target']
)
...
# MCC dağılımı
axes[0].hist(mccs, bins=30, edgecolor='k', color='tomato')
axes[0].set_title('Model 2 — MCC Dağılımı (1000 tekrar)')
axes[0].set_xlabel('MCC Değeri')
axes[0].set_ylabel('Frekans')

# Ortalama confusion matrix
mean_cm = np.mean(conf_matrices, axis=0).astype(int)
sns.heatmap(mean_cm, annot=True, fmt='d', cmap='Reds', ax=axes[1],
            xticklabels=list(CLASS_NAMES.values()),
            yticklabels=list(CLASS_NAMES.values()))
axes[1].set_title('Model 2 — Ortalama Confusion Matrix')
...
plt.show()
print("Grafik kaydedildi: model2_results.png")
