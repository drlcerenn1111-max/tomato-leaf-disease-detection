...
def toplu_karisiklik_matrisi_goster(all_cms, all_model_names, all_classes, all_asama_nos):
    """
    Tüm aşamaların confusion matrix'lerini tek bir figürde gösterir.
    """
    n    = len(all_cms)
    cols = 4
    rows = (n + cols - 1) // cols
    ...
    fig.suptitle("Cascade Classification - All Stages Summary", fontsize=14, fontweight="bold")
    plt.tight_layout()
...
def main():
    ...
    for asama_no, hedef_sinif in enumerate(unique_classes[:-1], start=1):
        ...
        # Toplu görsel için verileri sakla
